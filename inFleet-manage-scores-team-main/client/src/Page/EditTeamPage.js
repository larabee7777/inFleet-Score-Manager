...
import { useNavigate, useParams } from "react-router-dom"; // 6.0
import swal from "sweetalert";
import { Text, HStack, Box, Image, Button } from "@chakra-ui/react";
import { useSelector, useDispatch } from "react-redux";
import { updateScoreTeam } from "../Redux/actions/scoreTeamActions";
...
function EditTeamPage(props) {
  ...
  const onUpdateTeam = (teamInfo) => {
    ...
    if (sameNameChecker === true) {
      ...
      APIService.onUpdateScoreTeam(updateObj, (resTeam) => {
        if (resTeam.status === 200) {
          dispatch(updateScoreTeam(updateObj));
          navigate("/");
        } else {
